    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#btnImport").click(function () {
                $.jBox($("#importBox").html(), {
                    title: "Import Data", buttons: {"Close": true},
...
<%--&lt;%&ndash;			class="form-search" style="padding-left:20px;text-align:center;" onsubmit="loading('正在导入，请稍等...');"><br/>&ndash;%&gt;--%>
<%--			<input id="uploadFile" name="file" type="file" style="width:330px"/><br/><br/>　　--%>
<%--			<input id="btnImportSubmit" class="btn btn-primary" type="submit" value="   导    入   "/>--%>
<%--			<a href="${ctx}/sys/user/import/template">Downland the model</a>--%>
<%--&lt;%&ndash;			<a href="${ctx}/sys/user/import/template">下载模板</a>&ndash;%&gt;--%>
<%--		</form>--%>
<%--	</div>--%>
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/sys/user/check">user List</a></li>
...
<form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/user/check" method="post"
    ...
    <ul class="ul-form">
        ...
        <li><label>Office：</label><sys:treeselect id="company" name="company.id" value="${user.company.id}"
                                                  labelName="company.name" labelValue="${user.company.name}"
                                                  title="office" url="/sys/office/treeData?type=1"
                                                  cssClass="input-small" allowClear="true"/></li>
            <%--				title="公司" url="/sys/office/treeData?type=1" cssClass="input-small" allowClear="true"/></li>--%>
            <%--			<li><label>登录名：</label><form:input path="loginName" htmlEscape="false" maxlength="50" class="input-medium"/></li>--%>
        <li><label>Login Name：</label><form:input path="loginName" htmlEscape="false" maxlength="50"
                                                  class="input-medium"/></li>
        <li class="clearfix"></li>
            <%--			<li><label>归属部门：</label><sys:treeselect id="office" name="office.id" value="${user.office.id}" labelName="office.name" labelValue="${user.office.name}" --%>
        <li><label>Department：</label><sys:treeselect id="office" name="office.id" value="${user.office.id}"
                                                      labelName="office.name" labelValue="${user.office.name}"
                                                      title="Department" url="/sys/office/treeData?type=2"
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <tr>
        <th>Office</th>
        <th>Department</th>
        <th class="sort-column login_name">Login Name</th>
