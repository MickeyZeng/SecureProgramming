...
<%--		<li><a href="${ctx}/sys/role/">角色列表</a></li>--%>
		<li class="active"><a href="${ctx}/sys/role/assign?id=${role.id}"><shiro:hasPermission name="sys:role:edit">Allocated Role</shiro:hasPermission><shiro:lacksPermission name="sys:role:edit">People List</shiro:lacksPermission></a></li>
<%--		<li class="active"><a href="${ctx}/sys/role/assign?id=${role.id}"><shiro:hasPermission name="sys:role:edit">角色分配</shiro:hasPermission><shiro:lacksPermission name="sys:role:edit">人员列表</shiro:lacksPermission></a></li>--%>
	...
	<div class="container-fluid breadcrumb">
		<div class="row-fluid span12">
			<span class="span4">Role Name: <b>${role.name}</b></span>
<%--			<span class="span4">角色名称: <b>${role.name}</b></span>--%>
			<span class="span4">Office: ${role.office.name}</span>
<%--			<span class="span4">归属机构: ${role.office.name}</span>--%>
			<span class="span4">English Name: ${role.enname}</span>
<%--			<span class="span4">英文名称: ${role.enname}</span>--%>
		</div>
		<div class="row-fluid span8">
			<span class="span4">Role Type: ${role.roleType}</span>
<%--			<span class="span4">角色类型: ${role.roleType}</span>--%>
			<c:set var="dictvalue" value="${role.dataScope}" scope="page" />
			<span class="span4">Data Scope: ${fns:getDictLabel(dictvalue, 'sys_data_scope', '')}</span>
<%--			<span class="span4">数据范围: ${fns:getDictLabel(dictvalue, 'sys_data_scope', '')}</span>--%>
	...
	<div class="breadcrumb">
		...
		<input id="assignButton" class="btn btn-primary" type="submit" value="Allocated Role"/>
<%--		<input id="assignButton" class="btn btn-primary" type="submit" value="分配角色"/>--%>
		<script type="text/javascript">
			$("#assignButton").click(function(){
				top.$.jBox.open("iframe:${ctx}/sys/role/usertorole?id=${role.id}", "Allocated Role",810,$(top.document).height()-240,{
				<%--top.$.jBox.open("iframe:${ctx}/sys/role/usertorole?id=${role.id}", "分配角色",810,$(top.document).height()-240,{--%>
					buttons:{"Comfirm Allocated":"ok", "Delete choosen":"clear", "Delete":true}, bottomText:"Pass the office  and list the people in this role",submit:function(v, h, f){
					// buttons:{"确定分配":"ok", "清除已选":"clear", "关闭":true}, bottomText:"通过选择部门，然后为列出的人员分配角色。",submit:function(v, h, f){
						...
						if (v=="ok"){
							...
							if(pre_ids.sort().toString() == ids.sort().toString()){
								top.$.jBox.tip("Did not give 【${role.name}】To the new user！", 'info');
			...
			<tr>
				...
				<shiro:hasPermission name="sys:role:edit"><td>
					<a href="${ctx}/sys/role/outrole?userId=${user.id}&roleId=${role.id}" 
						onclick="return confirmx('Confirm delete the user<b>[${user.name}]</b>从<b>[${role.name}]</b>From the role list?？', this.href)">Delete</a>
